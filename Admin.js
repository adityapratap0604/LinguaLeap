...
app.post('/updateStudentProfile/:id', (req, res) => {
  ...
  app.post('/select',(req,res)=>{
    ...
    app.post('/teachersignup',(req,res)=>{
        ...
        const sql1='INSERT INTO teacherprofile (uname,email,mobileno,batch,languages,certification,location,Verification) VALUES (?,?,?,?,?,?,?,?)';
        connection.query(sql1,[uname,email,'','','','','','Not Verified'],(error,results,field)=>{
            if(error){
                console.error('Error inserting data into material : ',error);
                return;
            }
        ...
        connection.query(
          "SELECT Verification FROM teacherprofile WHERE email=? AND uname=?",
          [email, uname],
          (error, results) => {
            if (error) {
            ...
            if (results.length > 0) {
              res.json({ verification: results[0].Verification });
            } else {
